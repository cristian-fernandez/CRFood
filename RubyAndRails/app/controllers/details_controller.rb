class DetailsController < ApplicationController
  ...
  def index
    if params[:detail_filter]
      @details = Detail.where("updated_at > (?)",DateTime.parse(params[:detail_filter])).order('updated_at ASC')
    else
      @details = Detail.all.order('updated_at ASC')
    end
  end
  ...
  private
    ...
    def detail_params
      params.require(:detail).permit(:idFood, :description, :ingredients, :directions, :history)
    end
end
